...
export async function postVideoResults(
  ...
  try {
    ...
    const renameVideo = async () => {
      const pathFromPython = path.resolve(
        __dirname,
        '../uploads/results/output_video.mp4'
      )
      const extension = path.extname(pathFromPython)
      const newFileName = `output_${uuidv4()}${extension}`
      ...
      try {
        // Rename the file
        await fs.promises.rename(pathFromPython, newPath)
